            ...
            <div class="mt-10 col-lg-12">
                <v-data-table :headers="headers" :items="desserts" :search="search" sort-by="calories"
                        ...
                        <v-toolbar flat>
                            ...
                            <v-dialog v-model="dialog" max-width="450px">
                                ...
                                <!-- <template v-slot:activator="{ on, attrs }">
                                        ...
                                        <button class="newR mb-2 elevation-4" v-bind="attrs" v-on="on">
                                            Assign Dispatcher
                                        </button>
                                    </template> -->
                                <v-card>
                                    <v-card-title style="background-color: #21618C; color: #fff; margin-bottom: 1rem">
                                        <span class="text-h5">Assign a Rider</span>
                                    </v-card-title>
                                    <!-- <v-card-text> -->
                                    <div class="container">
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <v-text-field readonly v-model="editedItem.name" label="Client Name"
                                                    solo></v-text-field>
                                            </div>
                                            <div class="col-lg-12">
                                                <v-text-field readonly v-model="editedItem.date" label="Request Date"
                                                    solo></v-text-field>
                                            </div>
                                            <div class="col-lg-12">
                                                <v-text-field readonly v-model="editedItem.delivery"
                                                    label="Delivery Location" solo></v-text-field>
                                            </div>
                                            <div class="col-lg-12">
                                                <v-text-field readonly v-model="editedItem.client"
                                                    label="Pickup Location" solo></v-text-field>
                                            </div>
                                            <div class="col-lg-12">
                                                <v-text-field readonly v-model="editedItem.item" label="Items" solo>
                                                </v-text-field>
                                            </div>
                                            <div class="col-lg-12">
                                                <v-select :items="ridersSelect" v-model="driverD" value="_id"
                                                    label="Solo field" solo></v-select>
                                            </div>
                                        </div>
                                    </div>
                                    <v-card-actions>
                    ...
                    <template v-slot:[`item.rider`]="{ item }">
                        <span v-if="JSON.stringify(item.rider) === '{}'" small style="color: red" class="mr-2">
                            Not Assigned
                        </span>
                        <span v-else small style="color: green">
                            {{ item.rider.firstName }}
                        </span>
                    </template>
...
export default {
    data: () => ({
        ...
        messages: [],
        Items: [],
        ridersSelect: [{ value: null, text: 'Select Rider' }],
        // messages2: [],
        mesg: "",
        view: {
            topOfPage: true
    ...
    async mounted() {
        ...
        await axios.all([
            axios.get('http://localhost:3000/api/v1/jwtauth/request/requests/status', {
                headers: {
                    'token': `Bearer ${token}`
                }
            }),
            ...
            .then(axios.spread((response1, response2, response3) => {
                ...
                response1.data.data.adminrequests.forEach(elem => {
                    this.desserts.push({
                        requestedById: elem.requestedBy._id,
                        itemId: elem._id,
    ...
    methods: {
        ...
        async save() {
            const retrievedData = localStorage.getItem('token');
            const RuserId = this.editedItem.requestedById
            console.log(RuserId);

            // console.log(JSON.parse(retrievedData));
            const token = JSON.parse(retrievedData);
            // console.log(this.driverD);
            const riderId = this.driverD
            await axios.get(`http://localhost:3000/api/v1/jwtauth/riders/rider/${riderId}`,
                {
                    headers: {
                        'token': `Bearer ${token}`
                    }
                ...
                .then((response) => {
                    console.log(response.data.rider.firstName);
                    this.editedItem.rider = response.data.rider.firstName
                })
                .catch((error) => {
                    console.log(error);
                })
            ...
            if (this.editedIndex > -1) {
                await axios.patch(`http://localhost:3000/api/v1/jwtauth/request/request/${itemId}`,
                    ...
                    .then((response) => {
                        ...
                        Object.assign(this.desserts[this.editedIndex], this.editedItem.rider)
                    })
                    .catch((error) => {
                        console.log(error);
                    })
            ...
            // this.driverD = null
            await axios.post(`http://localhost:3000/api/v1/jwtauth/notification/user`,
                {
                    rider: this.driverD,
                    request: itemId,
                    requestedBy: RuserId
